...
def get_gpt_response(painting_info):
    ...
    prompt1 = "You are an art bot which helps interpret an artist's painting. Describe the painting from the artist perspective in under 1000 characters."
    prompt2 = "Imagine a conversation between the artist (prefix with their initials) and a viewer (prefix anon) of their painting. Describe the artist's thoughts, feelings, and intentions when creating the piece. Consider their journey and the challenges faced while creating this painting, and how they overcame those challenges to create a powerful and unique work of art."
...
def post_cast_chunks(gpt_response):
    # Break gpt_response into chunks of maximum 320 characters on completed words
    chunks = []
    start = 0
    end = 320
    while start < len(gpt_response):
        if end < len(gpt_response):
            while gpt_response[end] != ' ' and end > start:
                end -= 1
            if end == start:
                end = start + 320
        else:
            end = len(gpt_response)
        chunks.append(gpt_response[start:end].strip())
        start = end
        end += 320
...
def run_daily_cast():
    while True:
        ...
        if painting_info:
            ...
            if gpt_response:
                last_cast = post_cast_chunks(gpt_response)
...
def post_response_chunks(gpt_response, parent):
    # Break gpt_response into chunks of maximum 320 characters on complete sentences
    chunks = []
    start = 0
    end = 320
    while start < len(gpt_response):
        if end < len(gpt_response):
            while gpt_response[end] not in {'.', '!', '?'} and end > start:
                end -= 1
            if end == start:
                end = start + 320
            else:
    ...
    # Post the first chunk as a thread response
                ...
                if thread_cast and thread_cast.author.username == BOT_USERNAME:
                    ...
                    if query_cast_text.strip().endswith("?"):
                        ...
                        if not artbotica_liked:
                            ...
                            post_response_chunks(answer, notif.content.cast)
